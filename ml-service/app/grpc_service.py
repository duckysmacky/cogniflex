...
class MLAnalyzerServicer(ml_analyzer_pb2_grpc.MLAnalyzerServicer):
    ...
    def AnalyzePhoto(self, request, context):
        ...
        logging.info("[Photo] Data size: %s bytes (%.2f MB)", len(image_bytes), len(image_bytes) / 1024 / 1024)
    ...
    def AnalyzeVideo(self, request, context):
        ...
        logging.info("[Video] Data size: %s bytes (%.2f MB)", len(video_bytes), len(video_bytes) / 1024 / 1024)
...
def _reply_from_prediction(operation, start_time, confidence, pred):
    ...
    logging.info("[%s] Result: class=%s, confidence=%.4f", operation, predicted_class, confidence)
